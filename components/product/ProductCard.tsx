...
export default function ProductCard({ product, onClick }: ProductCardProps) {
  ...
  return (
    <div onClick={onClick} className="cursor-pointer">
      ...
      <div className="relative overflow-hidden rounded-t-lg">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all duration-300 flex items-end justify-center opacity-0 group-hover:opacity-100 z-20">
          <button
            ...
            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 text-sm transition-colors duration-200"
          >
            Find Similar
          </button>
        </div>
      ...
      <div className="p-3">
        <p className="text-sm text-gray-900 line-clamp-2 mb-2">{product.name}</p>
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="text-xs text-gray-500">{product.sold >= 1000 ? `${Math.floor(product.sold / 1000)}k` : product.sold} Sold</p>
        </div>
      </div>
    </Card>
    </div>
